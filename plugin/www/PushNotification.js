...
//Function: addAppId
//[android] Add new pushwoosh app. And register to it if pushToken got.
//
//Parameters:
// "appId" - Pushwoosh application code
// "projectid" - GCM project number for android platform
PushNotification.prototype.addAppId = function(appId, projectid) {
	exec(null, null, "PushNotification", "addAppId", [appId,projectid]);
};

//Function: removeAppId
//[android] Remove pushwoosh app. And unregister from pushwoosh.
//
//Parameters:
// "appId" - Pushwoosh application code
PushNotification.prototype.removeAppId = function(appId) {
	exec(null, null, "PushNotification", "removeAppId", [appId]);
};
...
//
//Parameters:
// "senderId" - project id which should be unregistered
// "success" - success callback
// "fail" - error callback
//
//Unregisters device from push notifications
PushNotification.prototype.unregisterDevice = function(senderId, success, fail) {
	exec(success, fail, "PushNotification", "unregisterDevice", [senderId]);
};
...
//
//Parameters:
// "appId" - app code for which tags will be send
...
//(start code)
//	pushwoosh.setTags("XXXXX-XXXXX", {deviceName:"hello", deviceId:10},
//		function() {
...
//(end)
PushNotification.prototype.setTags = function(appId, config, success, fail) {
	exec(success, fail, "PushNotification", "setTags", config ? [appId, config] : [appId]);
};
...
//
//Parameters:
// "appId" - app id for which pushToken needed
// "success" - getPushToken callback
//
//Example:
//(start code)
//	pushwoosh.getPushToken( "XXXXX-XXXXX",
//		function(token) {
//			console.warn('push token: ' + token);
//		}
//	);
//(end)
PushNotification.prototype.getPushToken = function(appId, success) {
	exec(success, null, "PushNotification", "getPushToken", appId);
};
...
//
//Parameters:
// "appId" - app id for which userId needed
// "userId" - user string identifier
//
PushNotification.prototype.setUserId = function(appId, userId) {
	exec(null, null, "PushNotification", "setUserId", [appId, userId]);
};
...
//
//Parameters:
// "appId" - app code for which postEvent will be send
// "event" - event to trigger
// "attributes" - object with additional event attributes
// 
// Example:
//(start code)
// pushwoosh.setUserId("XXXXX-XXXXX", "XXXXXX");
// pushwoosh.postEvent("XXXXX-XXXXX", "buttonPressed", { "buttonNumber" : 4, "buttonLabel" : "banner" });
//(end)
PushNotification.prototype.postEvent = function(appId, event, attributes) {
	exec(null, null, "PushNotification", "postEvent", [appId, event, attributes]);
};
