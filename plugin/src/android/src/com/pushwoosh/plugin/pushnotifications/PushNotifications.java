...
public class PushNotifications extends CordovaPlugin {
	...
	private boolean onDeviceReady(JSONArray data, CallbackContext callbackContext) {
		...
		try {
			...
			String appid = params.has("appid")? params.getString("appid"): params.getString("pw_appid");

			Pushwoosh.getInstance().addAppId(appid, params.getString("projectid"));
		} catch (Exception e) {
	...
	@CordovaMethod
	private boolean addAppId(JSONArray data, final CallbackContext callbackContext)
	{
		try {
			String appId = data.getString(0);
			String projectId = data.getString(1);
			Pushwoosh.getInstance().addAppId(appId, projectId);
		} catch (JSONException e) {
	...
	@CordovaMethod
	private boolean removeAppId(JSONArray data, final CallbackContext callbackContext)
	{
		try {
			String appId = data.getString(0);
			Pushwoosh.getInstance().removeAppId(appId);
		} catch (JSONException e) {
			PWLog.error(TAG, "No parameters passed (missing parameters)", e);
		}
		return true;
	...
	private boolean unregisterDevice(JSONArray data, CallbackContext callbackContext) {
		...
		try {
			String senderId = data.getString(0);
			Pushwoosh.getInstance().unregisterForPushNotifications(senderId, new Callback<String, UnregisterForPushNotificationException>() {
				@Override
	...
	private boolean setTags(JSONArray data, final CallbackContext callbackContext) {
		...
		Pushwoosh.getInstance().sendTags(appId, Tags.fromJson(params), new Callback<Void, PushwooshException>() {
			@Override
	...
	private boolean getPushToken(JSONArray data, final CallbackContext callbackContext) {
		try {
			callbackContext.success(Pushwoosh.getInstance().getPushToken(data.getString(0)));
		} catch (JSONException e) {
			e.printStackTrace();
			PWLog.error(TAG, "No parameters passed (missing parameters)", e);
			return false;
	...
	private boolean setUserId(JSONArray data, final CallbackContext callbackContext)
	{
		try
		{
			...
			PushwooshInApp.getInstance().setUserId(data.getString(0),userId);
		}
		catch (JSONException e)
		{
			PWLog.error(TAG, "No parameters passed (missing parameters)", e);
	...
	private boolean postEvent(JSONArray data, final CallbackContext callbackContext)
	{
		try
		{
			...
			JSONObject attributes = data.getJSONObject(2);
			PushwooshInApp.getInstance().postEvent(appId, event, Tags.fromJson(attributes));
		}
		catch (JSONException e)
		{
			PWLog.error(TAG, "No parameters passed (missing parameters)", e);
